...
export default function Edit() {
  ...
  const getImage = async (sku) => {
    const urls = [
      "https://testbronze.b-cdn.net/thinkbronze/new_picture/" + sku + "-.jpg",
      "https://testbronze.b-cdn.net/thinkbronze/new_picture/" + sku + "-.jpg",
      "https://testbronze.b-cdn.net/thinkbronze/new_picture/" + sku + "-.jpg",
      "https://testbronze.b-cdn.net/thinkbronze/new_picture/" + sku + "-.JPG",
      "https://testbronze.b-cdn.net/thinkbronze/new_picture/" + sku + "-.JPG",
    ...
    for (const url of urls) {
      ...
      if (exists) {
        return url; // ✅ break loop and return correct image
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/api/ebay-products/${id}`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const productById = async (id) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/ebay-products/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const sku_without_dash = "W_" + response.data.data.sku_without_dash;
      const url = await getImage(sku_without_dash);
      console.log("URL ++", url);
    } catch (error) {}
  };
