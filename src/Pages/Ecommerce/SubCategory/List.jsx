...
export default function List() {
  ...
  const getRecords = async (page) => {
    // handleLoader(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/api/sub-category`, {
        params: {
          page: page,
        },
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/api/sub-category/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      const result = await axios.post(
        `${API_BASE_URL}/api/sub-category/bulk_delete`,
        { ids: selectedIds },
        {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <div className="container-xxl flex-grow-1 container-p-y">
          ...
          <div className="card">
            <h5 className="card-header border-bottom">
              ...
              <Link
                to={`/admin/ecommerce/sub-category/create`}
                  ...
                  {brands && brands.length > 0 ? (
                    brands.map((brand, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <Link
                            to={`/admin/ecommerce/sub-category/${brand._id}`}
